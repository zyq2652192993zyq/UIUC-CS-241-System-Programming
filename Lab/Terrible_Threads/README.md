...
可以借助条件变量和互斥量来实现，测试程序就利用`LeetCode`的`1114.Print in Order`来进行测试。`LeetCode`的题目数组时`1,2,3`组成，我们利用这三个数字的全排列，可以构造6组测试用例，在C++里面有`next_permutation`，所以类似的，自己实现一个`algorithm`库，包含`swap, reverse, next_permutation`三个函数，这样就构造了六组测试用例。最后需要用`valgrind`检查是否存在内存泄漏，我们把它写在`memoryLeakCheck.sh`里面。

## `queue`

这个作业需要我们手动实现一个线程安全的队列这种数据结构。基本上按照`cookbook`里的内容来实现就可以了。对于队列容量的最大值，可以是正数，也可以是负数。如果是正数，则当队列已满，用户往队列里加入元素会被阻塞。如果是非正数，意味着往队列里面添加元素的时候永远不会阻塞。

测试的时候选择一种比较简单的测试，两个生产者，两个消费者，队列容量最大为10，最后检验队列的`size`是否为0。然后需要用`valgrind`去检验是否存在内存泄漏。

> 这个lab类似于`leetcode` 1118的设计有限阻塞队列，但是在 `leetcode` 这道题并没有支持C语言。
