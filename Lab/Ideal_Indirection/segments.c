...
void grow_segment(vm_segmentations *segmentations, segments segment, size_t num_pages) 
{
...
void shrink_segment(vm_segmentations *segmentations, segments segment, size_t num_pages) 
{
    vm_segmentation *segmentation = &(segmentations->segments[segment]);
    if (segmentation->grows_down) {
        segmentation->end += num_pages * PAGE_SIZE;
