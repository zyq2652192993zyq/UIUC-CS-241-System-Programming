...
addr32 ask_kernel_for_frame(page_table_entry *entry) {
    for (int i = 0; i < NUM_PHYSICAL_PAGES; i++) {
        if (entry_mapping[i] == NULL) {
            ...
            // Zeroing out the page, since an os does not want processes reading
...
/*
 * Necessary to interact with a 64 bit system using our 32 bit address space.
 *
 * Will convert the page table entry into a pointer that can be used to access
 * the memory
 */
 ...
 * Necessary to interact with a 64 bit system using our 32 bit address space.
 *
 * Will convert the page directory entry into a pointer that can be used to
 * access the memory
 */
 ...
 * Necessary to interact with a 64 bit system using our 32 bit address space.
 *
 * Will convert the 32 bit addr32 address into a pointer that can be used to
 * access the memory
 */
